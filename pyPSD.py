...
counts, realbins, air = plt.hist(extent, bins=bin_edges)  # create num dist from bin edges [1]
...
# creates a grid of scatterplots, per each column pair
    # if i != ext_col_idx+1:  # excludes the 'mirror' scatterplot
        plt.subplot(2, 2, i)
        plt.scatter(extent, dat[:, i-1], marker='.', c='black')
        plt.xlim(0, max(extent))
        plt.ylim(0, max(dat[:, i-1]))
        plt.xlabel(strs[i-1])
fig.set_figheight(8)
fig.set_figwidth(8)
...
plt.xlabel(volstr)
plt.ylabel('Volume')
plt.xticks(rotation=90)     # CHECK THAT THE AXIS OF THIS IS JUST WRONG AND NOT THE WHOLE PROGRAM
